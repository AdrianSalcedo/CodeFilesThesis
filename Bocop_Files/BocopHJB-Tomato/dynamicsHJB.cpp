...
#include "header_drift"
{
  ...
  double Nv = Sv+Iv;
  double Np = Sp+Lp+Ip;
  state_dynamics[0] = -betap*Sp*Iv/Nv +(r1+u1)*Lp +(r2+u2)*Ip;
  state_dynamics[1] = betap*Sp*Iv/Nv -(b+r1+u1)*Lp;
  state_dynamics[2] = b*Lp-(r2+u2)*Ip;
  state_dynamics[3] = -betav*Sv*Ip/Np-(gamma+gammaf+u3)*Sv+(1-theta)*mu;
  state_dynamics[4] = betav*Sv*Ip/Np-(gamma+gammaf+u3)*Iv+theta*mu;
  state_dynamics[5] = 0;
  state_dynamics[6] = 0;
}
...
#include "header_volatility"
{
  ...
  volatility_dynamics[0][0] = Sp*(sigmaL*Lp+sigmaI*Ip)/Np;
  volatility_dynamics[0][1] = 0;
  volatility_dynamics[1][0] = -sigmaL*Sp*Lp/Np;
  volatility_dynamics[1][1] = 0;
  volatility_dynamics[2][0] = -sigmaI*Sp*Ip/Np;
  volatility_dynamics[2][1] = 0;
  volatility_dynamics[3][0] = 0;
  volatility_dynamics[3][1] = -sigmaV*Sv;
  volatility_dynamics[4][0] = 0;
  volatility_dynamics[4][1] = -sigmaV*Iv;
