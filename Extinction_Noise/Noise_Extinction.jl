...
CSV.write(path * "Det_solution_noise_extinction_good.csv",det_DF1_red)
...
CSV.write(path * "Stc_solution_noise_extinction_good.csv",stc_DF1_red)
...
while j <= 10000
    ...
    Datos_aux = DataFrame(t = t_s, S_p = variables[:,1],
        I_p = variables[:,3], I_v = variables[:,5]) #only some variables
    ...
    condition5 = maximum(abs.(variables[end-150:end,3]))
