...
Data = CSV.read(path * "Data_noise_may_10.csv",DataFrame)
...
for i in 0:500
    ...
    Quartile_S_p = quantile!(Sub.S_p, [0.05, 0.25, 0.5, 0.75, 0.95])
    Quartile_I_p = quantile!(Sub.I_p, [0.05, 0.25, 0.5, 0.75, 0.95])
    Quartile_I_v = quantile!(Sub.I_v, [0.05, 0.25, 0.5, 0.75, 0.95])
    Data_aux = DataFrame(
    t = Mean_t,
...
CSV.write(path * "Data_Mean_Noise_Extinction.csv",Data_Mean)
