...
parameters {
    ...
    //real <lower = 1, upper = 10> Lp0;
    // real <lower = 0, upper= n_pop> Ip0;
    // real <lower = 0, upper= 50> Sv0;
    // real <lower = 0, upper= 25> Iv0;
    //real <lower = 0> N_v;
...
model {
    ...
    r_2 ~ uniform(0, 1);
    b ~ uniform(0, 1);
    beta_v ~ uniform(0,1);//normal(0.026894359,0.044418103);
    gamma ~ uniform(0, 1);
    gamma_f ~ uniform(0, 1);
    theta_1 ~ uniform(0, 1);
    mu ~ uniform(0, 1);
    ...
    y ~ neg_binomial_2(col(to_matrix(y_hat), 9), phi);
    //y ~ poisson(lambda);
