...
parameters {
    ...
    real <lower = 0, upper= 0.0005> Lp0;
    //real <lower = 0, upper= 0.01> Ip0;
    real <lower = 0, upper= 0.0001> Sv0;
    real <lower = 0, upper= 0.001> Iv0;
    //real <lower = 0> N_v;
...
model {
    real lambda[n_obs];      //gamboy dspoisson parameter
    //priors
    beta_p ~ normal(0.007570614,0.001291196);//normal(0.007570614,0.0001291196);
    //r_1 ~ uniform(0.0083, 0.013);
    //r_2 ~ uniform(0.0083, 0.013);
    //b ~ normal(0.01, 0.0001);
    beta_v ~ normal(0.026894359,0.044418103);//normal(0.0016894359,0.00044418103);
    //gamma ~ uniform(0.03, 1.0);
    //gamma_f ~ uniform(0.0001, 0.5);
    theta_1 ~ uniform(0.0001, 0.75);
    mu ~ uniform(0.25, 0.75);
    Lp0 ~ uniform(0.0, 0.0005);
    //Ip0 ~ uniform(0.0, 0.001);
    Sv0 ~ uniform(0.0, 0.0001);
    Iv0 ~ normal(0.001, 0.00001);
