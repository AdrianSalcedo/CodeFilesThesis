...
df_PSCL4 <- read_csv("/home/gabrielsalcedo/Documentos/TYLCVD-fit/model1/data/incidence_real_data.csv")
df_PSCL4_real <- read_csv("/home/gabrielsalcedo/Documentos/TYLCVD-fit/model1/data/incidence_real_data.csv")

onset_real <- df_PSCL4_real$Time
cum_cases_real <- df_PSCL4_real$y_incidence
...
n_chains <- 4
n_warmups <- 1250
n_iter <- 2500   #n_warmups + (n_iter * n_thin), total number of iterations 
n_thin <- 10
total_n_iter = n_warmups + (n_iter * n_thin)
#set.seed(972198)
...
nuts_fit <-
  sampling(model,
           ...
           chains = n_chains,
           warmup = n_warmups,
           iter = total_n_iter, #n_warmups + (n_iter * n_thin),
           thin = n_thin,
