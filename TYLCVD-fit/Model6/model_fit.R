...
model_fit <- function(fit = nuts_fit, dates = onset, dates_real = onset_real){
    ...
    df_sample_N = data.frame(cum_cases, dates$Time)
    ...
    write.csv(df_fit_CIS, "/home/gabrielsalcedo/Documentos/TYLCVD-fit/Model6/data/BinNeg.csv")
    #
    plt <- ggplot(df_sample_N,
                    aes(x = dates.Time, y = cum_cases)) +
            geom_ribbon(aes(x = dates.Time,
                            ymin = low_I,
                            ymax = high_I),
                        fill = "orange",
                        alpha = 0.6)  +
