...
function F_Det(du,u,p,t)
 @inbounds begin
        ...
        du[3] = b*u[2]-r_2*u[3]
        du[4] = -beta_v*u[4]*u[3]-(gamma + gamma_f)*u[4]+(1-theta)*mu/N_v
        du[5] = beta_v*u[4]*u[3]-(gamma + gamma_f)*u[5]+theta*mu/N_v
    end
    nothing
end
...
while j <= 10000
    ...
    Datos_aux = DataFrame(t = t_s,
        S_p = variables[:,1], I_p = variables[:,3], I_v = variables[:,5]) #only some variables
...
CSV.write(path * "Data_for_mean.csv",Datos)
