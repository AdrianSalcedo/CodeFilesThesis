...
circles = CSV.read(path0 * "datos_circle.csv", DataFrame)
squarts = CSV.read(path0 * "datos_squares.csv", DataFrame)
...
beta_p = 0.073080739#0.0351563
r_1 =  0.013#1.069008e-02#0.01#0.010404548#0.0090332
r_2 =  0.013#1.067034e-02#0.010813735#0.00952148
b =  5.0e-02#0.06#0.059934647#0.0625
beta_v = 1.000096e-03#0.009995286#0.0113525
theta = 0.4
mu = 1.0
gamma = 0.03
gamma_f = 0.03
...
#u_0 = [907.9833883/1000,87.01661167/1000,5/1000,1000/50000,40000/50000,7.576870231/1000]

T = 50.0
time = (7.0, T)
N_p = u_0[1] + u_0[2] + u_0[3]
dt = 0.01
t_s = range( 7.0, T, step = 1.0)
...
CSV.write(path0 * "Data_incidence_PSCL-4.csv",Datos)
