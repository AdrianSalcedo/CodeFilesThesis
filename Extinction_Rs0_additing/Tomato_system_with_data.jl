...
path1 = "/media/gabrielsalcedo/ADATA HV620S/"
path2 = "Julia_code_for_tomate_SDE_paper-main/Extinction_Rs0_additing/"
path = path1 * path2
...
R0_without_fumigation =
    (beta_p * beta_v * b) / ( gamma * ( b + r_1) * r_2)
...
CSV.write(path * "Det_Rs0_add_ext_Holt.csv",det_DF1_red)
...
det_DF1 = DataFrame(t = det_Time,S_p = Xu1, L_p =Xu2, I_p = Xu3, S_v = Xu4, I_v = Xu5)
det_DF1_red = det_DF1#[1:10:end,1:end]
CSV.write(path * "Det_Rs0_add_ext_good.csv",det_DF1_red)
...
CSV.write(path *"Stc_Rs0_add_ext_good.csv",stc_DF1_red)
